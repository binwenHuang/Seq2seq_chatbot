# Zakaria Daud Graduate Seq2seq chatbot project chat.py

# Program to startup chatbot
...
# Load dictionary to convert words to index from serial file
vocabs_to_index = pickle.load(open("vocab2index.p", "rb"))
index_to_vocabs = pickle.load(open("index2vocab.p", "rb"))

# Load hyperparameters and model weighths
batch_size = config.BATCH_SIZE
model_dir = config.MODEL_DIR
save_path = config.SAVE_PATH

# Create graph and import saved model weights from model
...
# Prompt user for input text and feed it into model then calculate and print output
state = True
while state:
